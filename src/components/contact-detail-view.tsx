...
import { AlertDialog, AlertDialogTrigger, AlertDialogCancel, AlertDialogContent, AlertDialogHeader, AlertDialogTitle, AlertDialogDescription, AlertDialogFooter } from '@/components/ui/alert-dialog';
...
export function ContactDetailView({ lead: initialLead, appSettings, salesCatalog, onLeadUpdate }: ContactDetailViewProps) {
  ...
  const [currentSchedule, setCurrentSchedule] = useState<CourseSchedule | null>(initialLead.courseSchedule || { sessionGroups: [] });

  // PayPlan state
  const [currentPayPlan, setCurrentPayPlan] = useState<PaymentPlan>(initialLead.paymentPlan || { totalPrice: 0, installments: [] });
  ...
  const handleEventManagement = async (action: 'Completed' | 'Cancelled' | 'Rescheduled') => {
    ...
    try {
        ...
        if (action === 'Rescheduled') {
            ...
            await updateDoc(doc(db, 'leads', lead.id), { interactions: updatedLead.interactions });
        ...
        const finalUpdatedLead = produce(lead, draft => {
        ...
        setLead(finalUpdatedLead);
        onLeadUpdate(finalUpdatedLead);
            ...
            <TabsContent value="overview" className="space-y-4 mt-4">
              <Card>
                  ...
                  <CardContent className="p-4 pt-0 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
                      ...
                      {lead.assignedAt && (
                        <div className="flex items-center gap-3">
                          <CircleUser className="h-4 w-4 text-muted-foreground flex-shrink-0" />
                          <p className="text-sm">Assigned on {format(parseISO(lead.assignedAt), "MMM d, yyyy")}</p>
                        </div>
                      )}
                  </CardContent>
              </Card>
